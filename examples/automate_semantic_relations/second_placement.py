...
def door_sampling(base: bproc.types.MeshObject, door: bproc.types.MeshObject, objects_boxes):
    ...
    for i in range(100):
        door.set_rotation_euler(base.get_rotation() + [0, 0, np.random.uniform(0, np.pi/2)]) # from 0 to pi/2.5
...
for ooi in objects_of_interest:
            ...
            # Take the path toobject to be placed
...
for obj in sample_surface_objects:
    ...
    with bproc.utility.UndoAfterExecution():
        ...
        for ooi in objects_of_interest[:-1]:
            ...
            sampling_obj = bproc.loader.load_obj(ooi["path"])
            ...
            dropped_object = place_object_on_object(sampling_obj, obj, bvh_cache, room_objs)

            if not dropped_object:
                continue


            if ooi["components"] == "door":
            ...
            next_object_surface = ooi

            # with open('examples/automate_semantic_relations/heights.txt', 'w') as f:
            #     height = np.max(obj.get_bound_box()[:,2]) 
            #     f.write( f"[{height}]")
            # f.close()

            # surface2 = bproc.object.extract_floor([dropped_object[0]], height_list_path="examples/automate_semantic_relations/heights.txt")
            ...
            while True:
                ...
                if next_object_surface["tags"][0]:# and np.random.randint(0, 1) == 1:

                    next_object_surface = objects_of_interest[1]#[np.random.randint(0, len(objects_of_interest))]

                    # for ii in range(np.random.randint(1, 6)):

                    sampling_obj = bproc.loader.load_obj(next_object_surface["path"])

                    print(sampling_obj[0].get_name())
                    print(dropped_object[0].get_name())

                    dropped_object = place_object_on_object(sampling_obj, dropped_object[0], bvh_cache, room_objs)
